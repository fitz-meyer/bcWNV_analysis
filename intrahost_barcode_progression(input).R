...
MG <- read_delim("cxt_mg_12dpi_9_S70_true_barcodes.txt", 
                      col_types = column_classes, col_names = c("barcode", "countMG"))
SG <- read_delim("cxt_sg_12dpi_9_S80_true_barcodes.txt", 
                       col_types = column_classes, col_names = c("barcode", "countSG"))
SL <- read_delim("cxt_sl_12dpi_9_S60_true_barcodes.txt", 
                       col_types = column_classes, col_names = c("barcode", "countSL"))
...
totalumis_MG <- sum(MG$countMG)
MG$freq_MG <- (MG$countMG)/totalumis_MG

totalumis_SG <- sum(SG$countSG)
SG$freq_SG <- (SG$countSG)/totalumis_SG

totalumis_SL <- sum(SL$countSL)
SL$freq_SL <- (SL$countSL)/totalumis_SL
...
MGandSG <- merge(MG, SG, by = "barcode", all.x = TRUE, all.y = TRUE) 
all_samples <- merge(MGandSG, SL, by = "barcode", all.x = TRUE, all.y = TRUE)
all_samples <- merge(all_samples, input, by = "barcode", all.x = TRUE, all.y = TRUE)
